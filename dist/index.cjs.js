...
class Model {
  ...
  reset() {
    this.data = lodash.clone(this.backup);
...
class Collection extends Model {
  ...
  constructor(name, data, index) {
    ...
    this.backup = lodash.clone(data);
    return new Proxy(this, {
      get: (obj, prop) => prop in obj.data ? {
        id: prop,
        ...obj.data[prop]
      } : obj[prop],
      set: (obj, prop, value) => {
        ...
        if (prop in obj.data) {
          ...
          obj.data[prop] = Object.assign(obj.data[prop], lodash.pick(value, keys));
        } else {
          obj.head = prop;
          obj.data[prop] = data;
        }
  ...
  add(data) {
    ...
    lodash.map(tmpl, (value, key) => {
      if (lodash.isFunction(value) && !(key in data)) {
        data[key] = value;
      }
...
lodash.isError = data => lodash.isObject(data) && lodash.has(data, 'status') && lodash.has(data, 'message') && data.status >= 400;

lodash.cast = id => {
  if (!lodash.isNaN(Number(id))) {
    id = Number(id);
  }
...
class Server {
  constructor(name) {
    this.name = name || 'mock-server'; // reformat data spec

    this.db = {};

    lodash.each(this.data(), (val, key) => {
  ...
  collection(options) {
    ...
    return {
      get: id => {
        let data = lodash.map(this.db[model].all(), item => lodash.omit(item, exclude));
  ...
  model(options) {
    ...
    return {
      get: id => {
        ...
        return lodash.omit(this.db[model].get(id), exclude);
      },
      put: (data, id) => {
        // with relation
        if (id && relation && key) {
  ...
  singleton(options) {
    ...
    return {
      get: () => lodash.omit(this.db[model].json(), exclude),
      put: data => lodash.omit(this.db[model].update(data), exclude),
  ...
  reset(model) {
    let obj = new this.constructor(); // reset everything

    if (model === undefined) {
      this.db = obj.db; // reset specific model
    } else if (model in obj.db) {
      this.db[model] = obj.db[model]; // handle invalid input
    } else {
